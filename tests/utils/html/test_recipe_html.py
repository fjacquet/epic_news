...
def test_recipe_renderer(sample_recipe_data):
    ...
    assert soup.find("h1", class_="recipe-title").text == "🍽️ Test Recipe"
    assert "1 cup of test ingredient" in soup.find("section", class_="recipe-ingredients").text
    assert "1. Test direction." in soup.find("section", class_="recipe-instructions").text
