...
def test_menu_renderer(sample_menu_data):
    ...
    assert "Test Menu" in soup.find("div", class_="menu-planner").find("h1", class_="menu-title").text
    assert "Test Breakfast" in soup.find("div", class_="day-plan").text
    assert "Test Category" in soup.find("section", class_="shopping-list").text
    assert "Test Item" in soup.find("section", class_="shopping-list").text
