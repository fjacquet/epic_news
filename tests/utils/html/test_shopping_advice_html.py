...
def test_shopping_renderer(sample_shopping_advice_data):
    ...
    assert "Test Product" in soup.find("section", class_="product-overview").text
    assert (
        "An amazing test product."
        in soup.find("section", class_="product-overview").text
    )
    # The recommendation is not directly in the renderer output, so we can't test for it here.
    # This is a limitation of the current renderer implementation.
