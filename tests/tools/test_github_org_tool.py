...
def mock_env_serper_only(): # Still need GITHUB_TOKEN for GitHubBaseTool init
    with patch.dict(os.environ, {"GITHUB_TOKEN": TEST_GITHUB_TOKEN, "SERPER_API_KEY": TEST_SERPER_API_KEY}, clear=True):
        yield
...
def test_run_github_api_success_with_repos(mock_make_request, mock_env_github_only):
    ...
    mock_org_response.json.return_value = {
        "login": "testorg", "name": "Test Organization", "html_url": "https://github.com/testorg",
        "description": "A test org", "public_repos": 10, "followers": 100, "repos_url": "https://api.github.com/orgs/testorg/repos"
    }
...
def test_run_github_api_success_no_repos_url(mock_make_request, mock_env_github_only):
    ...
    mock_org_response.json.return_value = {
        "login": "testorg", "name": "Test Organization", "html_url": "https://github.com/testorg",
        "description": "A test org", "public_repos": 10, "followers": 100, "repos_url": None # No repos URL
    }
...
def test_run_serper_fallback_success(mock_make_request, mock_os_getenv_in_tool_module, mock_env_serper_only):
    ...
    mock_os_getenv_in_tool_module.side_effect = lambda k, d=None: "" if k == "GITHUB_TOKEN" else os.environ.get(k, d)

    mock_serper_response = MagicMock()
    mock_serper_response.json.return_value = {
        "organic": [
            {"link": "https://github.com/testorg", "title": "Test Org on GitHub"}
        ]
    }
...
@patch('epic_news.tools.github_org_tool.os.getenv')
@patch('epic_news.tools.github_base.GitHubBaseTool._make_request')
def test_run_serper_fallback_not_found(mock_make_request, mock_os_getenv_in_tool_module, mock_env_serper_only):
    tool = GitHubOrgSearchTool()
    mock_os_getenv_in_tool_module.side_effect = lambda k, d=None: "" if k == "GITHUB_TOKEN" else os.environ.get(k, d)
...
@patch('epic_news.tools.github_org_tool.os.getenv')
@patch('epic_news.tools.github_base.GitHubBaseTool._make_request')
def test_run_serper_fallback_api_error(mock_make_request, mock_os_getenv_in_tool_module, mock_env_serper_only):
    tool = GitHubOrgSearchTool()
    mock_os_getenv_in_tool_module.side_effect = lambda k, d=None: "" if k == "GITHUB_TOKEN" else os.environ.get(k, d)
...
def test_run_no_keys_available(mock_env_no_keys):
    ...
    with patch.dict(os.environ, {"GITHUB_TOKEN": TEST_GITHUB_TOKEN, "SERPER_API_KEY": ""}, clear=True): # Init with GitHub token
        tool = GitHubOrgSearchTool()

    with patch.dict(os.environ, {"GITHUB_TOKEN": "", "SERPER_API_KEY": ""}, clear=True), patch('os.getenv') as mock_os_getenv:
