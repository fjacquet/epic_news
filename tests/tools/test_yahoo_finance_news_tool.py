...
from epic_news.tools.yahoo_finance_news_tool import YahooFinanceNewsTool, GetTickerNewsInput
...
def test_run_successful_news_retrieval_with_limit(mock_yf_ticker, tool_instance):
...
@patch('yfinance.Ticker')
def test_run_successful_news_retrieval_fewer_than_limit(mock_yf_ticker, tool_instance):
...
@patch('yfinance.Ticker')
def test_run_successful_news_retrieval_default_limit(mock_yf_ticker, tool_instance):
    ticker_symbol = "DEFAULTLIMITNEWS"
    mock_ticker_instance = MagicMock()
    mock_yf_ticker.return_value = mock_ticker_instance
    # Create 7 news items
    mock_ticker_instance.news = [
        {"title": f"News {i}", "publisher": "Pub", "link": f"link{i}.com", "providerPublishTime": int(datetime.datetime.now().timestamp())}
        for i in range(7)
    ]
...
@patch('yfinance.Ticker')
def test_run_no_news_found(mock_yf_ticker, tool_instance):
...
@patch('yfinance.Ticker')
def test_run_news_item_missing_fields(mock_yf_ticker, tool_instance):
    ticker_symbol = "MISSINGFIELDSNEWS"
    mock_ticker_instance = MagicMock()
    mock_yf_ticker.return_value = mock_ticker_instance
    mock_ticker_instance.news = [
        {"title": None, "publisher": "Pub D", "link": "link4.com", "providerPublishTime": None},
        {"title": "News E", "publisher": None, "link": None, "providerPublishTime": int(datetime.datetime.now().timestamp())}
    ]
    ...
    assert isinstance(result_data["news"][1]["published_date"], str) # Check it's a string, actual date will vary
...
@patch('yfinance.Ticker')
def test_run_with_zero_limit(mock_yf_ticker, tool_instance):
    ticker_symbol = "ZEROLIMITNEWS"
    mock_ticker_instance = MagicMock()
    mock_yf_ticker.return_value = mock_ticker_instance
    mock_ticker_instance.news = [
        {"title": "News 1", "publisher": "Pub A", "link": "link1.com", "providerPublishTime": int(datetime.datetime.now().timestamp())}
    ]
