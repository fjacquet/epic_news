...
class HolidayPlannerCrew:
    ...
    def _init_tools(self):
        ...
        combined_tools_list = (
            self.travel_research_tools + self.accommodation_search_tools + self.content_creation_tools
        )
        seen_tool_names = set()
        unique_tools = []
        for tool in combined_tools_list:
            if hasattr(tool, "name") and tool.name not in seen_tool_names:
                ...
                tool, "name"
            ):  # Handle tools without a name, though unlikely for BaseTool derivatives
                ...
                if (
                    tool not in unique_tools
                ):  # Fallback to object identity if no name - less reliable for de-duplication
                    unique_tools.append(tool)
